...
import { splitApiRequest, parseJsonParameter } from '../../transport';
...
export async function addTargetingRule(
  ...
  const ruleJson = this.getNodeParameter('rule', index) as string;
  const comments = this.getNodeParameter('comments', index, '') as string;
  const title = this.getNodeParameter('title', index, '') as string;

  const rule = parseJsonParameter(ruleJson, 'rule');
